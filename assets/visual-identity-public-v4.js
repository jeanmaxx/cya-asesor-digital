...
function detect(){const p=location.pathname.toLowerCase();if(p.includes('/admin/'))return null;if(p.includes('/barberias/'))return{kind:'beauty',table:'business_profiles',slug:qs.get('negocio')||'studio-cavalier',vertical:'beauty'};if(p.includes('/otros/'))return{kind:'other',table:'business_profiles',slug:qs.get('negocio')||'tu-tarjeta-digital',vertical:'other'};return{kind:'advisor',table:'advisor_profiles',slug:qs.get('asesor')||cfg.defaultSlug||'demo-publica'}}
