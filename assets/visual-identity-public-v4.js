...
function setFavicon(p){const src=p.logo_icon_path?storageUrl(p.logo_icon_path):(p.logo_path?storageUrl(p.logo_path):'');if(!src)return;let link=document.querySelector('link[rel="icon"]');if(!link){link=document.createElement('link');link.rel='icon';document.head.appendChild(link)}link.href=src;link.type='image/png'}
