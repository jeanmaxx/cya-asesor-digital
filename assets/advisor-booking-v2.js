...
function availableModes(){const s=ctx?.settings||{},m=[];if(s.allow_in_person&&(ctx.locations||[]).some(l=>l.appointment_enabled))m.push(['in_person','Presencial']);if(s.allow_phone)m.push(['phone','Llamada']);if(s.allow_video)m.push(['video','Videollamada']);if(s.allow_whatsapp)m.push(['whatsapp','WhatsApp']);return m}
...
async function submitBooking(e,root){
  ...
  const service=services.find(x=>x.id===payload.p_service_id)?.title||'Asesoría general';const location=(ctx.locations||[]).find(x=>x.id===locationId);const modeLabel={in_person:'Presencial',phone:'Llamada',video:'Videollamada',whatsapp:'WhatsApp'}[selectedMode];const when=`${mxDate(date)} · ${selectedSlot.start}`;const status=data?.status==='confirmed'?'Confirmada':'Pendiente de confirmación';const waText=`Hola, acabo de solicitar una asesoría desde tu tarjeta TTD.\n\n📌 ${service}\n📅 ${when}\n💬 Modalidad: ${modeLabel}${location?`\n📍 ${location.name}`:''}\n\nEstado: ${status}.`;
  const cal=calendarUrl(data.starts_at,data.ends_at,`Asesoría — ${profile?.first_names||'Asesor'} · ${service}`,`Solicitud registrada en TTD. Estado: ${status}. Modalidad: ${modeLabel}.`,location?.address||location?.name||modeLabel);
...
async function init(){
  ...
  const profileLookup=await supabase.from('advisor_profiles').select('id').eq('slug',slug).eq('is_published',true).maybeSingle();const advisorId=profileLookup.data?.id||'00000000-0000-0000-0000-000000000000';
  const [{data:context,error:ctxError},{data:p},{data:s}]=await Promise.all([
    ...
    supabase.from('advisor_services').select('id,title,is_visible,sort_order').eq('advisor_id',advisorId).eq('is_visible',true).order('sort_order')
  ]);
