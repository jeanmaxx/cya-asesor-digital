...
let pendingSave=false,visualDone=false,mainDone=false,savingPatch=false,profile=null;
...
async function inject(){
  if(!id||!$('other-form'))return;
  const config=document.querySelector('.vi-v4-config');
  if(!config||document.querySelector('.vi-text-config'))return false;
